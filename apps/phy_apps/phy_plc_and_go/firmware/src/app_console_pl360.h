/*******************************************************************************
  ...
  Description:
    ...
    "APP_CONSOLE_PL360_Initialize" and "APP_CONSOLE_PL360_Tasks" prototypes) 
    and some of them are only used internally by the application (such as the 
    "APP_CONSOLE_STATES" definition). Both are defined here for convenience.
*******************************************************************************/
...
typedef enum
{
    ...
    APP_CONSOLE_STATE_SET_PLC_CHANNEL,
...
/* Function:
  ...
  Precondition:
    APP_CONSOLE_PL360_Initialize must have returned a valid object handle.
  ...
  Example:
    ...
    if (result > MAX_VALUE)
    {
        APP_CONSOLE_PL360_Print("Result of %d exceeds max value\r\n", result);
    }
    </code>
