...
extern APP_PLC_DATA app_plcData;
extern APP_RF_DATA app_rfData;
...
void APP_DISPLAY_Tasks ( void )
{
    ...
    switch ( app_displayData.state )
    {
        ...
        {
            cl010_clear_all();
            cl010_show_numeric_string(CL010_LINE_DOWN, (const uint8_t *)"2150460");
        ...
        {
            if (app_plcData.state == APP_PLC_STATE_READY)
            {
                if (app_plcData.pvddMonTxEnable)
                {
            ...
            if (app_rfData.state == APP_RF_STATE_READY)
            {
